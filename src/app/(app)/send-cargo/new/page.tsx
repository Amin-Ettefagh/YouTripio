...
export default function SendCargoFormPage() {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    setTimeout(() => {
      setLoading(false)
      setToast({ show: true, message: t.forms.loadingSuccessCapacity, variant: "success" })
      setTimeout(() => setToast((prev) => ({ ...prev, show: false })), 2500)
    }, 900)
  }
  ...
  return (
    <div className="min-h-screen bg-[color:var(--color-bg)]">
      <DashboardHeader title={t.sendCargo.title} />
      ...
      <main className="section-shell py-12">
        <div className="glass mx-auto max-w-2xl rounded-3xl p-8">
          <h1 className="text-2xl font-semibold text-[color:var(--color-text)]">{t.forms.capacityAdTitle}</h1>
          <form onSubmit={handleSubmit} className="mt-6 space-y-5 text-sm text-[color:var(--color-muted)]">
            ...
            <div>
              <label className="text-xs uppercase tracking-[0.2em] text-[color:var(--color-muted)]">{t.forms.parcelType}</label>
              <input
                value={form.parcelType}
                onChange={(event) => updateField("parcelType", event.target.value)}
                placeholder={t.forms.parcelTypeHint}
                className="mt-2 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2"
              />
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link href="/send-cargo" className="text-sm text-[color:var(--color-muted)]">
                {t.forms.backToList}
              </Link>
            </div>
          </form>
