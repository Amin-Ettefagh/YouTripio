...
export default function SendCompanionFormPage() {
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    setTimeout(() => {
      setLoading(false)
      setToast({ show: true, message: t.forms.loadingSuccessCompanionOffer, variant: "success" })
      setTimeout(() => setToast((prev) => ({ ...prev, show: false })), 2500)
    }, 900)
  }
    ...
    <div className="min-h-screen bg-[color:var(--color-bg)]">
      ...
      <main className="section-shell py-12">
        <div className="glass mx-auto max-w-2xl rounded-3xl p-8">
          <h1 className="text-2xl font-semibold text-[color:var(--color-text)]">{t.forms.companionOfferTitle}</h1>
          <form onSubmit={handleSubmit} className="mt-6 space-y-5 text-sm text-[color:var(--color-muted)]">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Link href="/send-companion" className="text-sm text-[color:var(--color-muted)]">
                {t.forms.backToList}
              </Link>
            </div>
          </form>
