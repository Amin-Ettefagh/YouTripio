...
import { companionOffers } from "@/data/sample-data"

export default function SendCompanionListPage() {
  ...
  return (
    <div className="min-h-screen bg-[color:var(--color-bg)]">
      <DashboardHeader title={t.sendCompanion.title} />
      ...
      <main className="section-shell py-10">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-start">
          <div className="glass w-full max-w-sm rounded-2xl p-6">
            <h2 className="text-xl font-semibold text-[color:var(--color-text)]">{t.sendCompanion.heading}</h2>
            <p className="mt-2 text-sm text-[color:var(--color-muted)]">{t.sendCompanion.text}</p>
          ...
          <div className="flex-1">
            ...
            {showResults ? (
                ...
                {companionOffers.map((offer) => (
                  <div key={offer.id} className="glass rounded-2xl p-5 text-sm text-[color:var(--color-muted)]">
                    <div className="flex flex-wrap items-start justify-between gap-4">
                      ...
                      <div>
                        <p>{language === "fa" ? "صندلی" : "Seats"}: {offer.seats}</p>
                        <p>{t.common.vehicleType}: {offer.vehicle}</p>
                        <p>{t.common.price}: {offer.price}</p>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <p className="mt-6 text-sm text-[color:var(--color-muted)]">{t.sendCompanion.resultsHint}</p>
            )}
          </div>
        </div>
      </main>
